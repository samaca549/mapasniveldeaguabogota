...
import requests
import random
import json  # Este sí lo necesitas

# Obtener los datos simulados desde el servidor Flask
API_FLASK_URL = "http://127.0.0.1:5000/api/niveles"
try:
    response = requests.get(API_FLASK_URL)
    nivel_agua = response.json()
except:
    nivel_agua = []  # Por si no está activo el servidor
